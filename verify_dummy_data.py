...
def verify_dummy_data():
    ...
    # 1. Verify Kategori (5 records)
    kategori_count = Kategori.objects.count()
    print(f"1. Kategori: {kategori_count} records (Target: 5)")
    for k in Kategori.objects.all():
        print(f"   - {k.nama_kategori}")
    
    # 2. Verify Produk (5 records)
    ...
    # 3. Verify Pelanggan (5 records with specific requirements)
    ...
    # 5. Verify DiskonPelanggan (2 records)
    ...
    requirements_met.append(("Kategori records", kategori_count >= 5))
